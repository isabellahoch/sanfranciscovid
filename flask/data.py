...
new_zipcodes = ['94117','94134','94115']
...
# for zip in zipcodes:
#     results = client.get("tpyr-dvnc", id=zip, limit=2000)
#     if results:
#         results = results[0]
...
#         try:
#             new_worksheet = spreadsheet.add_worksheet(title=str(zip), rows="2", cols="20")
#             new_worksheet.update('B1', 'count')
#             new_worksheet.update('C1', 'rate')
#             new_worksheet.update('D1', '%')
#             new_worksheet.update('E1', 'deaths')
#             new_worksheet.update('F1', 'delta')
#             new_worksheet.update('G1', 'acs_population')
#             new_worksheet.update('H1', 'last_updated_at')
#             new_worksheet.update('I1', 'area_type')
#             new_worksheet.update('J1', 'id')
#             new_worksheet.update('A2', 'TOTAL')
...
#             new_worksheet.append_row([results['date'],results['count'],results['rate'],float(results['rate'])/100,results['deaths'],'-',results['acs_population'],results['last_updated_at'],results['area_type'],results['id']])
...
#             time.sleep(75)
#         except:
#             print("duplicate; "+str(zip)+" already exists.")
#     else:
#         print(results)


# new_worksheet = spreadsheet.sheet1
# results = running_total
# running_total["rate"] = float(results['count'])/float(results["acs_population"])*1000
...
# new_worksheet.update('F2', '-')
# new_worksheet.update('G2', results['acs_population'])
# new_worksheet.resize(2)
# new_worksheet.append_row([results['date'],results['count'],results['rate'],float(results['rate'])/100,results['deaths'],'-',results['acs_population'],results['date']])
# new_worksheet.format('A1:J1', {'textFormat': {'bold': True}})
# new_worksheet.format('A1:A', {'textFormat': {'bold': True}})
# sf_overall = new_worksheet.get_all_records()
sf_overall = []
print(sf_overall)